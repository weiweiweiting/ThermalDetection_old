...
#    cap = cv2.VideoCapture(video_path) # 抓取影像
...
#    writer = cv2.VideoWriter(output_video_name, fourcc, int(fps), (int(width), int(height))) # 將結果存起來
    total_frames = cap.get(cv2.CAP_PROP_FRAME_COUNT)
    if not cap.isOpened(): # 影片開啟失敗
...
if __name__ == "__main__":
    video_path = "3.mov";
    run_on_video(video_path, 'out.mov', conf_thresh=0.5)
